...
def load_images(image_paths, do_random_crop, do_random_flip, resize_w, crop_size, do_prewhiten=True):
	...
	for i in range(nrof_samples):
		...
		if(img.shape[1] <= 512):#generated images using augment_images
			...
			img = cv2.resize(img, (round(img.shape[0]*resize_w/img.shape[1]), resize_w))
		if img.ndim == 2:
		    img = to_rgb(img)
...
def augment_images(image_paths, crop_range = 4, rotate_range = [-10, 10], br_range = [-18, 18], contrast_range = [0.9, 1.7], do_flip = True, crop_size = 1024, resize_w = 512):
	nrof_samples = len(image_paths)
	for i in range(nrof_samples):
		s = image_paths[i]
		...
		for j in range(crop_range):
			...
			cv2.imwrite(path + '/aug/' + fn + '_' + str(j+1) + '_rc_b.jpg', img3)
			
			cv2.imwrite(path + '/aug/' + fn + '_' + str(j+1) + '_rc_bf.jpg', np.fliplr(img3))
...
def plot_graphs(loss, val_loss, acc, val_acc, wait_for_user_action = False):
	...
	ax1.plot(loss, 'blue', label='Training loss')
	ax1.plot(val_loss, 'red', label='Validation loss')
	...
	if(wait_for_user_action):
		...
		plt.pause(2)
	ax2.plot(acc, 'green', label='Training acc')
	ax2.plot(val_acc, 'orange', label='Validation acc')
